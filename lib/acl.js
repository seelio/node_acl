...
Acl.prototype.middleware = function(numPathComponents, userId, actions){
  ...
  return function(req, res, next){
    ...
    if (!userId) {
      if(!req.session.auth) {
        req.session.auth.id = 'guest';
      }
      _userId = req.session.auth.id; 
    }
    ...
    this.logger?this.logger.debug('Requesting '+actions+' on '+resource+' by user '+_userId):null;
    ...
    acl.isAllowed(_userId, resource, actions, function(err, allowed){
      if (err){
        ...
        this.logger?this.logger.debug('Not allowed '+actions+' on '+resource+' by user '+_userId):null;
        acl.allowedPermissions(_userId, resource, function(err, obj){
          this.logger?this.logger.debug('Allowed permissions: '+util.inspect(obj)):null;
        });
        next(new HttpError(401,'Insufficient permissions to access resource'));
      }else{
        this.logger?this.logger.debug('Allowed '+actions+' on '+resource+' by user '+_userId):null;
        next();
      }
    });
  };
